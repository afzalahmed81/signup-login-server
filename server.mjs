...
let userBase = []; // Data stored in a variable (Array), no database connected so far
...
app.post("/signup", (req, res) => {
    ...
    if (isFound) { // a check for the already existing emails
        res.status(400).send({
...
app.post("/login", (req, res) => {
    ...
    for (let i = 0; i < userBase.length; i++) {
        if (userBase[i].email === body.email) {
            ...
            isFound = true;
            if (userBase[i].password === body.password) { // For Correct password
